...
export function RegistrationForm({
  ...
  const minimumTeamSize = isTeam ? Math.max(minPlayers ?? 1, 1) : 1;

  const maximumTeamSize = isTeam
    ? Math.max(maxPlayers ?? minimumTeamSize, minimumTeamSize)
    : 1;

  const minimumAdditionalMembers = Math.max(minimumTeamSize - 1, 0);

  const [members, setMembers] = useState<Member[]>(
    Array.from({ length: minimumAdditionalMembers }, (_, index) => ({
      id: index + 1,
      name: "",
      studentCode: "",
    })),
  ...
  const totalPlayers = 1 + members.length;

  const canAddMember = isTeam && totalPlayers < maximumTeamSize;

  const canRemoveMember = isTeam && totalPlayers > minimumTeamSize;
  ...
  function addMember() {
    setMembers((current) => {
      if (1 + current.length >= maximumTeamSize) {
        return current;
      }
      const nextId =
        current.length > 0
          ? Math.max(...current.map((member) => member.id)) + 1
          : 1;
  ...
  function removeMember(memberId: number) {
    setMembers((current) => {
      if (1 + current.length <= minimumTeamSize) {
        return current;
      }
  ...
  function updateMemberName(memberId: number, value: string) {
    setMembers((current) =>
      current.map((member) =>
        member.id === memberId
          ? {
  ...
  function updateMemberStudentCode(memberId: number, value: string) {
    setMembers((current) =>
      current.map((member) =>
        member.id === memberId
          ? {
    ...
    >
      ...
      <div className="border-t border-[var(--border)] pt-6">
        <h2 className="font-bold text-[var(--text-primary)] mb-1">
          {isTeam ? "Captain Details" : "Participant Details"}
        </h2>

        <p className="text-sm text-[var(--text-muted)] mb-5">
          Provide the primary contact information for this registration.
        </p>
          ...
          <div className="flex items-start justify-between gap-4 mb-5">
            <div>
              <p className="text-sm text-[var(--text-muted)]">
                The captain is Player 1. This competition requires{" "}
                {minimumTeamSize === maximumTeamSize
                  ? `${minimumTeamSize} players`
                  : `${minimumTeamSize}–${maximumTeamSize} players`}
                .
              </p>

              <p className="mt-2 text-xs font-semibold text-[var(--text-secondary)]">
                {totalPlayers} / {maximumTeamSize} players
              </p>

              <p className="text-sm text-[var(--text-muted)]">
                Add the remaining players in your team. The captain is already
                counted as Player 1.
              </p>
            </div>
            ...
            <button
              type="button"
              onClick={addMember}
              disabled={isPending || state.success || !canAddMember}
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateMemberStudentCode(member.id, event.target.value)
                      }
                      placeholder="Student ID or roll number"
                      className="w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-4 py-3 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--border-strong)] disabled:opacity-60"
                    />
      ...
      <div className="border-t border-[var(--border)] pt-6">
        ...
        <p className="text-xs text-[var(--text-muted)] mt-3">
          Registration will require approval from the REFLEX organizers.
        </p>
      </div>
    </form>
  );
